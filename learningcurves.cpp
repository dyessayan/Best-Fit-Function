...
long double rmse_fitness(Node * n, std::vector<double> cases, std::vector<double> targets, const int&timeout = 300) {
    long double error = 0;
...
long double rmse_fitness(Node * n, std::vector<double> cases, std::vector<double> targets, double parsimony_coefficient, const int& timeout=300) {
    auto start_time = std::chrono::high_resolution_clock::now();
...
int main() {
    ...
    std::vector<long double> coefficient_values {1, 0.1, 0.01, 0.001, 0.0001}; //0.01
...
for(int p = 0; p < parameter_count; p++) {
    ...
    parameters *best_params_for_best_fitness;
    parameters *best_params_for_fair_fitness;
    ...
    for(int repeat = 0; repeat < num_repeats; repeat++) {
        ...
        while(random_generation < generations_at_termination.at(j).at(repeat)) {
            ...
            if(random_generation % CHECKPOINT_FREQUENCY == 0) {
                std::string random_output = ".\\Checkpoints\\Random\\" + std::to_string(run_id) + "_Gen_" + std::to_string(random_generation) + ".txt";
...
if (DO_HILLCLIMBER) {
    int j = 0;
        for(int repeat = 0; repeat < num_repeats; repeat++) {
            ...
            std::string hc_learning_curve = ".\\Outputs\\HillClimber\\" + std::to_string(run_id) + std::to_string(run_id) + "_Repeat_" + std::to_string(repeat) + "_learning_curve.txt";
